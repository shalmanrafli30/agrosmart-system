...
//     fetchJSON(`${API_URL}/api/dashboard?site_id=${userSiteId}`).then((resData: DataResponse | null) => {
//       if (!resData) return;
...
//         if (d && d[0]?.value_status && ['Warning', 'Danger'].includes(d[0].value_status)) {
//           warning.push({
...
//     fetchJSON(`${API_URL}/api/realtime?site_id=${userSiteId}`).then((realtimeData: Partial<DataResponse> | null) => {
...
//       setActionMessages(prev => [...prev, ...warningSensors]);
...
// Final Revisi Dashboard.tsx dengan fix looping dan filter sensor realtime
